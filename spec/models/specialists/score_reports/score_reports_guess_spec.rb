describe ScoreReportsGuess do
  describe '.all_between' do
    it "returns all guesses between the given dates" do
      guess = create :score_reports_guess, added_at: Time.utc(2011, 1, 1, 0, 0, 1)
      expect(ScoreReportsGuess.all_between(Time.utc(2011), Time.utc(2011, 1, 1, 0, 0, 1))).to eq([guess])
    end

    it "ignores guesses made on or before the from date" do
      create :score_reports_guess, added_at: Time.utc(2011)
      expect(ScoreReportsGuess.all_between(Time.utc(2011), Time.utc(2011, 1, 1, 0, 0, 1))).to eq([])
    end

    it "ignores guesses made after the to date" do
      create :score_reports_guess, added_at: Time.utc(2011, 1, 1, 0, 0, 2)
      expect(ScoreReportsGuess.all_between(Time.utc(2011), Time.utc(2011, 1, 1, 0, 0, 1))).to eq([])
    end
  ...
  describe '#years_old' do
    it "returns the number of full years from post to guess (ignoring leap years)" do
      photo = ScoreReportsPhoto.new dateadded: Time.utc(2010)
      guess = ScoreReportsGuess.new photo: photo, commented_at: Time.utc(2011)
  ...
  describe '#seconds_old' do
    it "returns the number of full seconds from post to guess" do
      photo = ScoreReportsPhoto.new dateadded: Time.utc(2010)
      guess = ScoreReportsGuess.new photo: photo, commented_at: Time.utc(2010, 1, 1, 0, 0, 1)
  ...
  describe '#star_for_age' do
    ...
    expected.keys.sort.each do |year_guessed|
      it "returns a #{expected[year_guessed]} star for a #{year_guessed - 2000}-year-old guess" do
        photo = ScoreReportsPhoto.new dateadded: Time.utc(2000)
        guess = ScoreReportsGuess.new photo: photo, commented_at: Time.utc(year_guessed)
  ...
  describe '#star_for_speed' do
    ...
    expected.keys.sort.each do |seconds_guessed|
      it "returns a #{expected[seconds_guessed]} star for a #{seconds_guessed}-second-old guess" do
        photo = ScoreReportsPhoto.new dateadded: Time.utc(2000)
        guess = ScoreReportsGuess.new photo: photo, commented_at: Time.utc(2000) + seconds_guessed
