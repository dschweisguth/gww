describe MultiPhotoMapSupport do
  describe '#as_map_json' do
    it "returns JSON-ready data including the given indication of whether or not the list is partial, bounds and photos" do
      photo = build :photos_photo, latitude: 37, longitude: -122, color: Color::Yellow.scaled(0, 0, 0), symbol: '?'
      bounds = Bounds.new 1, 3, 2, 4
      partial = false
      expect(PhotosPhoto.as_map_json(partial, bounds, [photo])).to eq(
        partial: partial,
        bounds: bounds,
        photos: [
          {
    ...
    it "moves younger photos so that they don't completely overlap older photos with identical locations" do
      photos = build_list :photos_photo, 2, latitude: 37, longitude: -122, color: Color::Yellow.scaled(0, 0, 0), symbol: '?'
      bounds = Bounds.new 1, 3, 2, 4
      partial = false
      photo_json = PhotosPhoto.as_map_json(partial, bounds, photos)[:photos]
  ...
  describe '#perturb_identical_locations' do
    it "moves younger photos so that they don't completely overlap older photos with identical locations" do
      photos = build_list :photos_photo, 3, latitude: 37, longitude: -122
      PhotosPhoto.perturb_identical_locations photos
