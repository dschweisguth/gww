describe PeoplePerson do
  describe '.autocompletions' do
    let(:person) { create :people_person }

    it "matches a username" do
      expect(PeoplePerson.autocompletions(person.username.slice(0, 3))).
        to eq([{ value: person.username, label: person.username_and_realname }])
    end

    it "matches a real name" do
      expect(PeoplePerson.autocompletions(person.realname.slice(0, 3))).
        to eq([{ value: person.username, label: person.username_and_realname }])
    end

    it "doesn't match if it shouldn't" do
      expect(PeoplePerson.autocompletions("abc")).to eq([])
    end
  ...
  describe '.find_by_multiple_fields' do
    ...
    it "finds a person by username" do
      expect(PeoplePerson.find_by_multiple_fields(person.username)).to eq(person)
    end

    it "finds a person by flickrid" do
      expect(PeoplePerson.find_by_multiple_fields(person.flickrid)).to eq(person)
    end

    it "finds a person by GWW ID" do
      expect(PeoplePerson.find_by_multiple_fields(person.id.to_s)).to eq(person)
    end

    it "punts back to the home page" do
      expect(PeoplePerson.find_by_multiple_fields('xxx')).to be_nil
    end
  ...
  describe '.nemeses' do
    it "lists guessers and their favorite posters" do
      guesser, favorite_poster = make_potential_favorite_poster(:people, 10, 15)
    ...
    it "ignores less than #{Person::MIN_GUESSES_FOR_FAVORITE} guesses" do
      make_potential_favorite_poster(:people, 9, 15)
      expect(PeoplePerson.nemeses).to eq([])
    end
  ...
  describe '.top_guessers' do
    ...
    it "returns a structure of scores by day, week, month and year" do
      ...
      guess = create :people_guess, commented_at: report_time
      (0..3).each { |division| expected[division][0].scores[1] = [guess.person] }
      expect(PeoplePerson.top_guessers(report_time)).to eq(expected)
    end

    it "handles multiple guesses in the same period" do
      expected = expected_periods_for_one_guess_at_report_time
      guesser = create :people_person
      create :people_guess, person: guesser, commented_at: report_time
      create :people_guess, person: guesser, commented_at: report_time + 1.minute
    ...
    it "handles multiple guessers with the same scores in the same periods" do
      expected = expected_periods_for_one_guess_at_report_time
      guess1 = create :people_guess, commented_at: report_time
      guess2 = create :people_guess, commented_at: report_time
    ...
    it "handles previous years" do
      ...
      guess = create :people_guess, commented_at: Time.local(2010, 1, 1).getutc
  ...
  describe '#username_and_realname' do
    ...
    it "returns 'username' for a user that has no real name" do
      person = build :people_person, realname: nil
    ...
    it "returns 'username' for a user whose username and real name are identical" do
      person = build :people_person, username: "the same string", realname: "the same string"
      expect(person.username_and_realname).to eq(person.username)
    end
  ...
  describe '#paginated_commented_photos' do
    ...
    it "sorts the most recently updated photos first" do
      photo1 = create :people_photo, lastupdate: 2.days.ago
      create :comment, photo: photo1, flickrid: person.flickrid, username: person.username
      photo2 = create :people_photo, lastupdate: 1.days.ago
