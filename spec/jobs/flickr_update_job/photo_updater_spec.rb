module FlickrUpdateJob
  describe PhotoUpdater do
    describe '.update_all' do
      ...
      def stub_get_faves
        allow(FaveCounter).to receive(:count).with('incoming_photo_flickrid').and_return(7)
        7
      end

      def mock_get_comments_and_tags
        allow(CommentUpdater).to receive(:update).with an_instance_of(FlickrUpdatePhoto)
        allow(TagUpdater).to receive(:update).with an_instance_of(FlickrUpdatePhoto)
        yield
        expect(CommentUpdater).to have_received(:update)
        expect(TagUpdater).to have_received(:update)
      end
      ...
      it "propagates errors other than that which comes from an invalid datetime" do
        ...
        expect { PhotoUpdater.update_all }.to raise_error(ArgumentError, "some other error")
      end
      ...
      it "updates only seen_at, views and faves if Flickr says the photo hasn't been updated" do
        ...
        stubbed_faves = stub_get_faves
        allow(CommentUpdater).to receive(:update)
        allow(TagUpdater).to receive(:update)
        person = create :person, flickrid: 'incoming_person_flickrid'
        photo_before = create :flickr_update_photo,
          person: person,
          flickrid: stubbed_photo[:id],
          ...
          faves: 6
        PhotoUpdater.update_all
        expect(CommentUpdater).not_to have_received(:update)
        expect(TagUpdater).not_to have_received(:update)
      ...
      it "sets a new photo's faves to 0 if the request for faves fails" do
        stub_get_photos
        allow(FaveCounter).to receive(:count).and_return(nil)
        mock_get_comments_and_tags do
          PhotoUpdater.update_all
        end
    ...
    describe '.update' do
      ...
      it "loads the photo and its person, location, faves, comments and tags from Flickr" do
        ...
        allow(FaveCounter).to receive(:count).with(photo.flickrid).and_return(7)
        allow(CommentUpdater).to receive(:update).with photo
        allow(TagUpdater).to receive(:update).with photo
      ...
      it "handles a photo with no location information" do
        stub_get_person
        stub_get_photo
        allow(LocationGetter).to receive(:get).with(photo.flickrid).and_return([nil, nil, nil])
        allow(FaveCounter).to receive(:count).with(photo.flickrid).and_return(7)
        allow(CommentUpdater).to receive(:update).with(photo)
        allow(TagUpdater).to receive(:update).with photo
        PhotoUpdater.update photo
      ...
      it "moves on if there is an error getting faves" do
        ...
        stub_get_photo
        stub_get_photo_location
        allow(FaveCounter).to receive(:count).with(photo.flickrid).and_return(nil)
        allow(CommentUpdater).to receive(:update).with(photo)
        allow(TagUpdater).to receive(:update).with photo
        PhotoUpdater.update photo
      ...
      it "only updates the photo's seen_at and the person if the photo hasn't been updated, for performance" do
        ...
        allow(LocationGetter).to receive(:get)
        allow(FaveCounter).to receive(:count)
        allow(CommentUpdater).to receive(:update)
        allow(TagUpdater).to receive(:update)
        PhotoUpdater.update photo

        expect(LocationGetter).not_to have_received(:get)
        expect(FaveCounter).not_to have_received(:count)
        expect(CommentUpdater).not_to have_received(:update)
        expect(TagUpdater).not_to have_received(:update)
        expect(photo.person).to have_attributes?(
          username: 'new_username',
      ...
      def stub_get_photo_location
        allow(LocationGetter).to receive(:get).with(photo.flickrid).and_return([37.123456, -122.654321, 16])
      end
