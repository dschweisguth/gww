describe 'SearchParamsParser' do
  describe '#form_params' do
    before do
      allow(Person).to receive(:exists?).with(username: 'known-username').and_return(true)
    end
    ...
    it "removes an unknown username from the segments" do
      allow(Person).to receive(:exists?).with(username: 'unknown-username').and_return(false)
