describe PeopleController do
  ...
  describe 'autocomplete_usernames' do
    it { is_expected.to have_named_route :autocomplete_usernames, "/autocomplete_usernames" }
    it { is_expected.to have_named_route :autocomplete_usernames, 'foo', "/autocomplete_usernames/foo" }
  ...
  describe 'find' do
    it { is_expected.to have_named_route :find_person, '/people/find' }
  ...
  %w{ nemeses top_guessers }.each do |action|
    describe action do
      it { is_expected.to have_named_route "#{action}_people", "/people/#{action}" }
      it { is_expected.to route(:get, "/people/#{action}").to action: action }
    end
  end

  describe 'show' do
    it { is_expected.to have_named_route :person, 666, '/people/666' }
    it { is_expected.to route(:get, '/people/666').to action: 'show', id: '666' }
  end

  %w{ guesses map map_json }.each do |action|
    describe action do
      it { is_expected.to have_named_route "person_#{action}", 666, "/people/666/#{action}" }
      it { is_expected.to route(:get, "/people/666/#{action}").to action: action, id: '666' }
    end
  end

  describe 'comments' do
    it { is_expected.to have_named_route :person_comments, 666, 1, '/people/666/comments/page/1' }
