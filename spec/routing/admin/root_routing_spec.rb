describe Admin::RootController do
  ...
  describe 'index' do
    it { is_expected.to have_named_route :admin_root, '/admin' }
  ...
  %w{ update_from_flickr calculate_statistics_and_maps }.each do |action|
    describe action do
      it { is_expected.to have_named_route action, "/admin/#{action}" }
      it { is_expected.to route(:post, "/admin/#{action}").to controller: 'admin/root', action: action }
    end
  end

  describe 'bookmarklet' do
    it { is_expected.to have_named_route :admin_root_bookmarklet, '/admin/bookmarklet' }
